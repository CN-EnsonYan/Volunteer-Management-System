...
mysql_close();//关闭数据库
//记录成功并开始警告
    header("Location:/apps/vms/account/login/index.html?type=al&ref=https%3A%2F%2Fvolunteer.ensonyan.com%2main%2Fadmin&2Faction-rej.php?token=FoqXwKt0T59jDXrP");exit;
}
...
$uid = $_GET['uid'];
// 更新数据
mysql_query("UPDATE works SET status='已驳回' WHERE uid=$uid",$link) or die('修改数据出错：'.mysql_error());
mysql_query("UPDATE works SET getcredit='0' WHERE uid=$uid",$link) or die('修改数据出错：'.mysql_error());
header("Location:response/reject-succeed.php");