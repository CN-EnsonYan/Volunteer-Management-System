...
//记录成功并开始警告
	...
	echo "
        <script>
             setTimeout(function(){window.location.href='/apps/vms/account/login/index.html?type=al&ref=https%3A%2F%2Fvolunteer.ensonyan.com%2main%2Fadmin%2Faction-conf.php?token=FoqXwKt0T59jDXrP';},100);
        </script>";
        exit;  
}
...
// 修改的uid
$uid = $_GET['uid'];
$gusername = safeStrings($_GET['username']);
$username = $_SESSION['entry_name'];
// 更新数据
mysql_query("UPDATE works SET status='已通过' WHERE uid=$uid",$link) or die('修改数据出错：'.mysql_error());
mysql_query("UPDATE works SET getcredit = wantcredit WHERE username = '$gusername' and status = '已通过'",$link) or die('修改数据出错：'.mysql_error());
header("Location:response/confirm-succeed.php");