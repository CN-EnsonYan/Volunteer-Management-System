...
//记录成功并开始警告
	...
	echo "
        <script>
             setTimeout(function(){window.location.href='/apps/vms/account/login/index.html?type=al&ref=https%3A%2F%2Fvolunteer.ensonyan.com%2main%2Fadmin&2Faccount-stop.php?token=FoqXwKt0T59jDXrP';},100);
        </script>";
        exit;  
}
...
$id = $_GET['id'];
// 更新数据
mysql_query("UPDATE user SET verify='已认证' WHERE id=$id",$link) or die('修改数据出错：'.mysql_error()); 
header("Location:response/rnauth-conf.php");