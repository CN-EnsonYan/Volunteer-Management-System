...
<?php
    ...
    if(!isset($_POST['submit'])){
		...
		header("Refresh:2;url=/apps/vms/account/login/index.html?type=sc&stat=rgfailed&token=???");
    }//判断是否有submit操作
...
while($row = mysql_fetch_array($rssysstat))
               ...
               if($row['status'] == 'running'){
                  ...
                  mysql_close();//关闭数据库
echo "<script>
        setTimeout(function(){window.location.href='/apps/vms/system-unavailable.html?type=st&dt=rej&ref=https%3A%2F%2Fvolunteer.ensonyan.com%2Faccount%2Flogin%2Fauth.php&token=???';},100);
     </script>";
        exit;
				  }
			  }
...
echo '<script type="text/javascript">alert("Registration function has been turned off by the Administrator. / 注册功能已被管理员暂时关闭");history.go(-1);</script>;';
  ...
  if ($homeroom == 'Homeroom') {
    ...
    if(isset($name))
    {
        ...
        if(mysql_query($sql))
        {
            echo '<script type="text/javascript">alert("Registration success. / 注册成功，谢谢您的关注！");window.location.href="/apps/vms/account/login/index.html?from=vms_reg_main-op&token=???";</script>';
        }else{
            echo '<script type="text/javascript">alert("Registration failed, please contact xxx @ xxx. / 注册失败，请您及时与 xxx 联系 (QQ:xxx)，谢谢配合！");window.location.href="/apps/vms/account/register/sign-up.html?type=failed";</script>';
        }
    }
